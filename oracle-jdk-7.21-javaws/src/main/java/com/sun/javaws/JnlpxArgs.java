...
public class JnlpxArgs
{
  ...
  static List getArgumentList(String paramString, long paramLong1, long paramLong2, JVMParameters paramJVMParameters, boolean paramBoolean1, JfxRuntime paramJfxRuntime, boolean paramBoolean2, int paramInt)
  {
    ...
    String str2 = StringQuoteUtil.unquoteIfNeeded(getVMArgs());
    ...
    String str6;
    if (str7 == null)
      str6 = "-Xbootclasspath/a:" + Environment.getDeploymentHome() + File.separator + "lib" + File.separator + "javaws.jar" + File.pathSeparator + Environment.getDeploymentHome() + File.separator + "lib" + File.separator + "deploy.jar" + File.pathSeparator + Environment.getDeploymentHome() + File.separator + "lib" + File.separator + "plugin.jar";
    else
    ...
    {
      str8 = "-Djnlp.fx=" + paramJfxRuntime.getProductVersion().toString();
      if (!paramBoolean2)
        str9 = "-Djnlp.tk=awt";
    }
    String[] arrayOfString = { str6, str8, str9, "-classpath", Environment.getDeploymentHome() + File.separator + "lib" + File.separator + "deploy.jar", null, str3 != null ? "-Xdock:icon=" + str3 : "", str4 != null ? "-Xdock:name=" + str4 : "", "-Djnlpx.jvm=" + paramString, "-Djnlpx.splashport=" + getSplashPort(), "-Djnlpx.home=" + Environment.getDeploymentHome() + File.separator + "bin", "-Djnlpx.remove=" + (shouldRemoveArgumentFile() ? "true" : "false"), "-Djnlpx.offline=" + (isOffline() ? "true" : "false"), "-Djnlpx.relaunch=true", "-Djnlpx.session.data=" + (str5 != null ? str5 : ""), str1, "-Djava.security.policy=" + getPolicyURLString(), "-DtrustProxy=true", "-Xverify:remote", useJCOV(), useBootClassPath(), useJpiProfile(), useDebugMode(), useDebugVMMode(), paramBoolean2 ? "" : "-Dsun.awt.warmup=true", "com.sun.javaws.Main", paramBoolean1 ? "-secure" : "", _verbose ? "-verbose" : "", setTCKHarnessOption(), Environment.isWebJava() ? "" : "-notWebJava", useLogToHost() };
    for (int i = 0; i < arrayOfString.length; i++)
      if (arrayOfString[i] != null)
        paramInt -= arrayOfString[i].length() + 1;
    List localList = paramJVMParameters.getCommandLineArguments(false, false, true, paramBoolean1, false, paramInt / 2 - 20);
    ...
    String str10 = StringQuoteUtil.getStringByCommandList(localList);
    paramInt -= str10.length();
    Object localObject2 = new Property("jnlpx.vmargs", str10);
    String str11 = ((Property)localObject2).toString(true);
    if (paramInt < str11.length())
    {
      Trace.println("JnlpxArgs.getArgumentList: Internal Error:  remaining custArgsMaxLen: " + paramInt + " < vmArgsPropertyStr.length: " + str11.length() + " dropping vmArgsPropertyStr");
      str11 = null;
    }
    for (int k = 0; k < arrayOfString.length; k++)
      ...
      else if (arrayOfString[k].length() > 0)
  ...
  {
    String str1 = Environment.getDeploymentHome() + File.separator + "lib" + File.separator + "security" + File.separator + "javaws.policy";
    String str2 = str1;
    try
    {
  ...
  public static Process execProgram(JREInfo paramJREInfo, String[] paramArrayOfString, long paramLong1, long paramLong2, JVMParameters paramJVMParameters, boolean paramBoolean1, JfxRuntime paramJfxRuntime, boolean paramBoolean2)
    throws IOException
  {
    String str1 = null;
    String str2 = null;
