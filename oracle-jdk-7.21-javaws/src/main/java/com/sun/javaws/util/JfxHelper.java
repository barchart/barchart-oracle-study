...
public class JfxHelper
{
  ...
  public static boolean isCompatibleRuntime(JREInfo paramJREInfo1, JREInfo paramJREInfo2)
  {
    if ((paramJREInfo1.getFXVersion() == null) && (paramJREInfo2.getFXVersion() == null))
      return true;
    return (!paramJREInfo1.getProductVersion().isGreaterThan(paramJREInfo2.getProductVersion())) && (!paramJREInfo1.getFXVersion().isGreaterThan(paramJREInfo2.getFXVersion()));
  }
    ...
    throws Throwable
  {
    ...
    Resource localResource = ResourceProvider.get().getResource(paramURL, null);
    ...
    JfxRuntime localJfxRuntime = Platform.get().getStandaloneJfxRuntime();
    if (null == localJfxRuntime)
      throw new IllegalStateException("Installed JavaFX runtime cannot be located.");
    if (paramString != null)
    {
      VersionString localVersionString = new VersionString(paramString);
      if (!DefaultMatchJRE.isFXVersionMatch(localJfxRuntime.getProductVersion().toString(), localVersionString))
  ...
  public static boolean isSupportedOS(String paramString)
  {
    for (int i = 0; i < SUPPORTTED_OS.length; i++)
      if (SUPPORTTED_OS[i].indexOf(paramString) != -1)
        return true;
    return false;
  }
    ...
    throws LaunchDescException
  {
    ...
    if ((!isSupportedJreVersion(localVersionID)) && (!localVersionID.match(JRE_MINIMUM_VER)))
    {
