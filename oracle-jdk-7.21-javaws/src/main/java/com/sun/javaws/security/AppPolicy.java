...
public class AppPolicy
{
    ...
    throws ExitException
  {
    ...
    if (paramBoolean)
      if (i == 0)
      {
        grantRestrictedAccess(localLaunchDesc, paramCodeSource, paramJNLPClassLoaderIf.getPreloader());
      }
      else
      {
  ...
  public void grantRestrictedAccess(LaunchDesc paramLaunchDesc, CodeSource paramCodeSource, Preloader paramPreloader)
    throws ExitException
  {
    AppInfo localAppInfo = paramLaunchDesc.getAppInfo();
    try
    {
      SandboxSecurity.isPermissionGranted(paramCodeSource, localAppInfo, paramPreloader);
    }
    catch (Exception localException)
    {
      throw new ExitException("Sandbox permissions not granted", localException, 0);
    }
    ...
    throws ExitException
  {
    ...
    if (i == 0)
    {
      ...
      if ((paramLaunchDesc.getAppletDescriptor() != null) || (paramLaunchDesc.getApplicationDescriptor() != null))
      {
        ...
        {
          localObject = ResourceProvider.get().getLocalApplicationProperties(paramLaunchDesc.getCanonicalHome(), null, true);
