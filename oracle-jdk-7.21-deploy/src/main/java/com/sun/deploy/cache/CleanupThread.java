...
class CleanupThread extends Thread
{
    ...
    for (int i = 0; i < 64; i++)
    {
      ...
      if ((localFile2.exists()) && (localFile2.isDirectory()))
      {
        File[] arrayOfFile = localFile2.listFiles(new FileFilter()
        {
          ...
          {
            String str = paramAnonymousFile.getName().toLowerCase();
  ...
  private Object[] prepareRemovalList(ArrayList paramArrayList)
  {
    ...
    Arrays.sort((Object[])localObject, new Comparator()
    {
      ...
      {
        CacheEntry localCacheEntry1 = (CacheEntry)paramAnonymousObject1;
        CacheEntry localCacheEntry2 = (CacheEntry)paramAnonymousObject2;
  ...
  public void run()
  {
    while (true)
      try
      {
        ...
        Trace.println(ResourceManager.getMessage("console.trace.plugin.applet.cleanup") + l2 + " us", TraceLevel.NETWORK);
      }
