...
final class NSRegistry
{
    ...
    throws IOException
  {
    ...
    {
      for (i = paramRecord.down; i != 0; i = localRecord.left)
      {
    ...
    for (int i = paramRecord.value; i != 0; i = localRecord.left)
    {
      localRecord = readRecord(i);
      if (str.equals(readString(localRecord.name, localRecord.namelen - 1)))
    ...
    throws IOException
  {
    ...
    for (int i = localRecord1.down; i != 0; i = readRecord(i).left)
      dump(paramString + "    ", i);
    Record localRecord2;
    for (int j = localRecord1.value; j != 0; j = localRecord2.left)
    {
      localRecord2 = readRecord(j);
