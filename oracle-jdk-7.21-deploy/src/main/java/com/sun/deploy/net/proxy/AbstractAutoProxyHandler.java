  ...
  implements ProxyHandler
{
  ...
  {
    URL localURL = null;
    Trace.msgNetPrintln("net.proxy.auto.download.ins", new Object[] { paramURLConnection.getURL() });
    String str = null;
    try
    {
      INIFile localINIFile = new INIFile(paramURLConnection.getInputStream());
    ...
    {
      throw new ProxyConfigException("Malformed URL specified in INS file: " + str, localMalformedURLException);
    }
    catch (Throwable localThrowable)
    {
      throw new ProxyConfigException("Unable to obtain INS file from " + paramURLConnection.getURL(), localThrowable);
    }
    return localURL;
  }
  ...
  private String getJSFileFromURL(URLConnection paramURLConnection)
    throws ProxyConfigException
  {
    Trace.msgNetPrintln("net.proxy.auto.download.js", new Object[] { paramURLConnection.getURL() });
    try
    {
      RemoveCommentReader localRemoveCommentReader = new RemoveCommentReader(new InputStreamReader(paramURLConnection.getInputStream()));
    ...
    {
      throw new ProxyConfigException("Unable to obtain auto proxy file from " + paramURLConnection.getURL(), localThrowable);
    }
  }
  ...
  private static final class INIFile
  {
    ...
    {
      String str1 = null;
      BufferedReader localBufferedReader = new BufferedReader(new InputStreamReader(this._inputStream));
      try
      {
        int i = 0;
        String str2 = null;
