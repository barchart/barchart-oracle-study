...
public class NSPreferences
{
  ...
  private static boolean getDefaultValueOfUseSystemPrefs()
  {
    ...
    bool = (getDeaultValue(localFile1)) || (getDeaultValue(localFile2));
    return bool;
  }
    ...
    try
    {
      ...
      while ((str1 = localBufferedReader.readLine()) != null)
        if (str1.startsWith("user_pref"))
          ...
          else if (isKeyword(str1, "network.proxy.http"))
          {
          ...
          else if (isKeyword(str1, "network.proxy.http_port"))
          {
            j = parseInt(str1);
            Trace.netPrintln("    network.proxy.http_port=" + j);
            paramBrowserProxyInfo.setHttpPort(j);
          }
          else if (isKeyword(str1, "network.proxy.ssl"))
          {
          ...
          else if (isKeyword(str1, "network.proxy.ssl_port"))
          {
            int k = parseInt(str1);
            Trace.netPrintln("    network.proxy.ssl_port=" + k);
            paramBrowserProxyInfo.setHttpsPort(k);
          }
          else if (isKeyword(str1, "network.proxy.ftp"))
          {
          ...
          else if (isKeyword(str1, "network.proxy.ftp_port"))
          {
            int m = parseInt(str1);
            Trace.netPrintln("    network.proxy.ftp_port=" + m);
            paramBrowserProxyInfo.setFtpPort(m);
          }
          else if (isKeyword(str1, "network.proxy.gopher"))
          {
          ...
          else if (isKeyword(str1, "network.proxy.gopher_port"))
          {
            int n = parseInt(str1);
            Trace.netPrintln("    network.proxy.gopher_port=" + n);
            paramBrowserProxyInfo.setGopherPort(n);
          }
          else if (isKeyword(str1, "network.proxy.socks"))
          {
          ...
          else if (isKeyword(str1, "network.proxy.autoconfig_url"))
          {
          ...
          else if (isKeyword(str1, "config.use_system_prefs"))
          {
    ...
    if (paramFile.exists())
    {
      ...
      while ((str2 = localBufferedReader.readLine()) != null)
        if (str2.trim().toLowerCase().equals("[profile0]"))
          j = 1;
        else if ((j != 0) && (str2.toLowerCase().startsWith("isrelative=")))
          try
          {
        ...
        else if ((j != 0) && (str2.toLowerCase().startsWith("path=")))
