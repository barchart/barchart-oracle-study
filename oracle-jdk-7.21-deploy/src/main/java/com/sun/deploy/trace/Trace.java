...
public class Trace
{
  ...
  private static void firePrintlnEvent(TraceMessage paramTraceMessage)
  {
    ...
    synchronized (traceListeners)
    {
      arrayOfTraceListener = (TraceListener[])traceListeners.toArray(new TraceListener[traceListeners.size()]);
    }
      ...
      if (paramFile != null)
      {
        ...
        if ((i > 0) && (arrayOfFile.length >= i))
        {
          ...
          Arrays.sort(arrayOfFile, new Comparator()
          {
            ...
            {
              Date localDate1 = new Date(((File)paramAnonymousObject1).lastModified());
              Date localDate2 = new Date(((File)paramAnonymousObject2).lastModified());
    ...
    public void run()
    {
      ...
      while (true)
      {
        ...
        if (localTraceMessage != null)
          Trace.firePrintlnEvent(localTraceMessage);
      }
    }
