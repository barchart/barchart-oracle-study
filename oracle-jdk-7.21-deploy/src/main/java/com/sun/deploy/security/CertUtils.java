...
public class CertUtils
{
    ...
    throws CertificateException, IOException
  {
    ...
    if (Config.isJavaVersionAtLeast18())
      localBoolean = (Boolean)localNetscapeCertTypeExtension.get(paramString);
    else
      localBoolean = callPre18NetscapeCertTypeExtensionGet(localNetscapeCertTypeExtension, paramString);
    ...
    throws IOException
  {
    ...
    else
    {
      ...
      if (localObject[0] == 4)
        localObject = new DerValue((byte[])localObject).getOctetString();
      Trace.msgSecurityPrintln(extractSubjectAliasName(paramX509Certificate));
  ...
  private static Boolean callPre18NetscapeCertTypeExtensionGet(NetscapeCertTypeExtension paramNetscapeCertTypeExtension, String paramString)
  {
    return (Boolean)AccessController.doPrivileged(new PrivilegedAction()
    {
      ...
      public Object run()
      {
        try
        {
          Method localMethod = CertUtils.class$sun$security$x509$NetscapeCertTypeExtension.getDeclaredMethod("get", new Class[] { String.class });
          Object localObject = localMethod.invoke(this.val$extn, new Object[] { this.val$type });
  ...
  protected static boolean add(KeyStore paramKeyStore, String paramString1, Certificate paramCertificate, String paramString2, boolean paramBoolean)
  {
    String[] arrayOfString = getExistingAliases(paramKeyStore, paramCertificate);
    if (arrayOfString != null)
      try
      {
        for (int i = 0; i < arrayOfString.length; i++)
        {
          ...
          if (matchingAlias(str2, paramString2))
          {
            if ((!paramBoolean) || (str2.indexOf("$tsflag") > -1))
              return true;
            paramKeyStore.deleteEntry(str2);
          }
        }
      }
      catch (KeyStoreException localKeyStoreException1)
      {
        Trace.ignored(localKeyStoreException1);
        return false;
      }
    String str1 = null;
    while (true)
    {
      str1 = paramString1 + (paramBoolean ? "$tsflag" : null) + (paramString2 != null ? "$loc=" : null) + paramString2 + new Random();
      try
      {
        if (paramKeyStore.getCertificate(str1) == null)
        {
          paramKeyStore.setCertificateEntry(str1, paramCertificate);
          return true;
        }
      }
      catch (KeyStoreException localKeyStoreException2)
      {
        Trace.ignored(localKeyStoreException2);
      }
    }
    return false;
  }

  protected static boolean contains(KeyStore paramKeyStore, Certificate paramCertificate, String paramString, boolean paramBoolean)
  {
    String[] arrayOfString = getExistingAliases(paramKeyStore, paramCertificate);
    if (arrayOfString != null)
      for (int i = 0; i < arrayOfString.length; i++)
      {
        ...
        if (matchingAlias(str, paramString))
          if (paramBoolean)
          {
            if (str.indexOf("$tsflag") >= 0)
              return true;
          }
          else
            return true;
  ...
  private static String[] getExistingAliases(KeyStore paramKeyStore, Certificate paramCertificate)
  {
    ArrayList localArrayList = new ArrayList();
    try
    {
      ...
      while (localEnumeration.hasMoreElements())
      {
        ...
        if (paramCertificate.equals(paramKeyStore.getCertificate(str)))
          localArrayList.add(str);
      }
    }
    catch (KeyStoreException localKeyStoreException)
    {
      Trace.ignored(localKeyStoreException);
    }
    return (String[])localArrayList.toArray(new String[0]);
  }

  private static boolean matchingAlias(String paramString1, String paramString2)
  {
    if (paramString1.indexOf("$loc=") < 0)
      return true;
    return paramString1.indexOf("$loc=" + paramString2) >= 0;
  }
}
