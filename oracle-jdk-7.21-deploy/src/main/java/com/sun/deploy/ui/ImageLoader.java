  ...
  implements Runnable
{
    ...
    while (i == 0)
    {
      ...
      if (i == 0)
        try
        {
          ...
          if (localURL == null)
          {
            Resource localResource = ResourceProvider.get().getResource(localLoadEntry.iconRef, localLoadEntry.iconVer, false, 0, null);
  ...
  private class DelayedImageLoader extends Thread
  {
    ...
    public void run()
    {
      try
      {
        Resource localResource = ResourceProvider.get().getResource(this._entry.iconRef, this._entry.iconVer);
        ...
        ImageLoader.publish(this._entry, this._image, localFile, true);
      }
      catch (MalformedURLException localMalformedURLException)
      {
        Trace.ignoredException(localMalformedURLException);
